...
var edgeTestDll = process.env.EDGE_USE_CORECLR
  ? "test"
  : path.join(__dirname, "Edge.Tests.dll");
var prefix = process.env.EDGE_USE_CORECLR
  ? "[CoreCLR]"
...
describe("edge.func", function() {
  ...
  it(prefix + " fails with a unsupported language parameter", function() {
    assert.throws(function() {
      edge.func("idontexist", "somescript");
  ...
  it(prefix + " fails with both assemblyFile or source", function() {
    assert.throws(function() {
      edge.func({
        assemblyFile: "foo.dll",
        source: "async (input) => { return null; }"
      });
  ...
  it(
    prefix + " succeeds with assemblyFile, type name, and method name",
    function() {
      var func = edge.func({
        assemblyFile: edgeTestDll,
        typeName: "Edge.Tests.Startup",
  ...
  it(
    prefix + " fails with assemblyFile, type name and nonexisting method name",
    function() {
      assert.throws(function() {
        edge.func({
          assemblyFile: edgeTestDll,
